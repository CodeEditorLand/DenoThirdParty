...
async function getBinariesInfo(
	...
	const downloadUrl = `https://github.com/denoland/deno_lint/releases/download/${tag}/`;
	return [
		{
			filename: "linux64.zip",
			url: new URL("dlint-x86_64-unknown-linux-gnu.zip", downloadUrl),
		},
		{
			filename: "mac.zip",
...
async function downloadAndSaveBinary(
	...
	if (!response.ok) {
		throw new Error(
			`error on fetch ${binary.url}: ${await response.text()}`,
		);
	}
